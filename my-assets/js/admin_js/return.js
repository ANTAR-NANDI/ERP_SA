...
function checkboxcheck(sl) {
  ...
  if ($("#" + check_id).prop("checked") == true) {
    ...
    document.getElementById(total_pricehidden).setAttribute("name", "total_price[]");
...
function addInputField(t) {
  ...
  if (count == limits)
    ...
    e.innerHTML = "<td><input type='text' name='product_name' onkeypress='invoice_productList(" + count + ");' class='form-control productSelection common_product' placeholder='Product Name' id='" + a + "' required tabindex='" + tab1 + "'><input type='hidden' class='common_product autocomplete_hidden_value  product_id_" + count + "' name='re_product_id[]' id='SchoolHiddenId'/></td>" +
      ...
      "<td class='text-right' hidden><input class=' re_vat form-control text-right' type='text' name='re_vat[]' id='re_vat_" + count + "' name='re_vat[]' value='0.00' readonly='readonly'/></td>" +
      "<td class='text-right' hidden><input class=' re_tax form-control text-right' type='text' name='re_tax[]' id='re_tax_" + count + "' name='re_tax]' value='0.00' readonly='readonly'/></td>" +
...
function quantity_calculate_re(item) {
  ...
  var re_vat = ($("#re_vat_" + item).val() ? $("#re_vat_" + item).val() : 0);
  var re_tax = ($("#re_tax_" + item).val() ? $("#re_tax_" + item).val() : 0);
  ...
  var row_tot = ((just_tot) - ((just_tot) * (discount / 100)) - ((just_tot) * (comm_item / 100))) + parseInt(re_tax) + parseInt(re_vat);
...
function calculateSum() {
  ...
  $("#re_total_vat").val(vat);
  $("#re_total_tax").val(tax);
...
"use strict";
function invoice_productList_new(sl) {
  var csrf_test_name = $('[name="csrf_test_name"]').val();
  var base_url = $("#base_url").val();
...
function quantity_calculateSreturn(item) {
  ...
  var price_item = $("#price_item_" + item).val();
  var discount = $("#discount_" + item).val();
  var price_discount = $("#price_discount_" + item).val();
  if (parseInt(sold_qty) < parseInt(quantity)) {
    alert("Purchase quantity less than quantity!");
    $("#total_qntt_" + item).val("");
  }
  if (parseInt(quantity) > 0) {
    var disc = ( price_discount /sold_qty ) * quantity;
    // alert(disc)
    // $("#price_discount_" + item).val(disc);
    var price = quantity * price_item;
    var dis = price * (discount / 100);
    $("#all_discount_" + item).val((dis ? dis : 0));
    // console.log(dis)


    //Total price calculate per product
    $("#itemwise_discount_" + item).val((disc ? disc : 0));
    var temp = price - ((dis ? dis : 0) + (disc ? disc : 0));
    ...
    $(".total_discount").each(function () {
      ...
      $(".itemwise_discount").each(function () {
        isNaN(this.value) || tt == this.value.length || (tt += (parseFloat(this.value) ? parseFloat(this.value) : 0));
      }),
      $("#total_itemwise_discount").val(tt.toFixed(2, 2));
  }
}
