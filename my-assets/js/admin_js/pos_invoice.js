...
function quantity_calculate(item) {
    ...
    if(total_vat_permission == 'disable')
    {
        ...
        $("#vat_percent_" + item).val(0);
    }
    if(total_tax_permission == 'disable')
    {
        $("#tax_percent_" + item).val(0);
    }
    var tax_percent = $("#tax_percent_" + item).val();
    ...
    $("#tax_" + item).val((tax_percent * ((quantity * price_item) - discount)) /100);
    $("#vat_" + item).val((vat_percent * ((quantity * price_item) - discount)) /100);
    ...
    //   alert(parseFloat(vat).toFixed(2))
   ...
   // console.log('vat',vat,'just total',just_tot);
    var row_tot = ((just_tot) - discount - ((just_tot) * (comm_item / 100)));
    // var row_tot = ((just_tot) - ((just_tot) * (discount / 100)) - ((just_tot) * (comm_item / 100))) + parseInt(tax) + parseInt(vat);
     
     let points = $("#points_" + item).val() ? $("#points_" + item).val() : 0
    //  alert(points)
     let product_percentage_discount  = $("#product_percentage_discount_" + item).val()
     let product_points_discount  = $("#product_points_discount_" + item).val()
    //  alert(product_points_discount)
     $("#product_percentage_discount_amount_" + item).val(row_tot.toFixed(2, 2) * product_percentage_discount/100)
     $("#product_burned_points_" + item).val((row_tot.toFixed(2, 2) * product_percentage_discount/100) * product_points_discount)
    //  product_percentage_discount_amount
    //  console.log(row_tot + " " + points);
    // alert(product_percentage_discount)

    // alert(row_tot+ " " +points)
    if(points == 0)
    {
        $("#total_points_" + item).val(0)
    }
    else{
        $("#total_points_" + item).val(Math.floor(row_tot.toFixed(2, 2)/points))
    }

    $("#total_price_wd_" + item).val(just_tot - discount);
...
function calculateSum() {
    ...
    var t = 0,
        ...
        tax = 0,
        total_points = 0,
        discounted_points_amount = 0,
        product_burned_points = 0,
    ...
    $(".vat").each(function () {
        ...
        // Points Sum of Prducts
        $(".total_points").each(function () {
            isNaN(this.value) || 0 == this.value.length || (total_points += parseFloat(this.value))
        }),
        $(".product_percentage_discount_amount").each(function () {
            isNaN(this.value) || 0 == this.value.length || (discounted_points_amount += parseFloat(this.value))
        }),
        $(".product_burned_points").each(function () {
            isNaN(this.value) || 0 == this.value.length || (product_burned_points += parseFloat(this.value))
        }),
    ...
    $(".total_price").each(function () {
            ...
            var distributed_discount = ((ttl_discount / t) * main_discounted_price) ? ((ttl_discount / t) * main_discounted_price) : 0;
            console.log(main_discounted_price + " " + distributed_discount )
            ...
            if(total_vat_permission == 'disable')
            {
                ...
                $("#vat_percent_" + newStr).val(0);
            }
            if(total_tax_permission == 'disable')
            {
                $("#tax_percent_" + newStr).val(0);
            }
    ...
    var total_price_with_total_discount = t - ttl_discount;

    $("#total_price_with_total_discount").val(total_price_with_total_discount.toFixed(2, 2));
    // alert(tax)
    $("#total_vat").val(vat);
    $("#total_tax").val(tax);
    $("#total_points").val(total_points.toFixed(2, 2));
    $("#discounted_points_amount").val(discounted_points_amount.toFixed(2, 2));
    ...
    $("#points_burn").val(product_burned_points.toFixed(2, 2));
    $("#max_discounted_points_amount").val((parseFloat((discounted_points_amount * max_points_burn))/product_burned_points.toFixed(2, 2)).toFixed(2, 2));
    console.log("HIII"+available_points + " " + point_eligibility)
    if(parseFloat(available_points) >= parseFloat(point_eligibility))
    {
        if(available_points >= product_burned_points)
        {
            $("#max_points_burn").val(product_burned_points);
        }
        else
        {
            $("#max_points_burn").val(available_points);
        }
    ...
    var test = +s_cost + + vat + + tax + + service_charge + + t + -ds + + ad - commission - pds;
    var test2 = +s_cost + + vat + + tax + + service_charge + + t + -ds + + ad - pds;
    console.log("service" + s_cost + "vat" + vat+"tax" + tax+"service_charge" 
    + service_charge+"t" + t+"ds" + ds+"ad" + ad + "commission" + commission +"pds" + pds)
...
function customer_due(id) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $("#membership_id").val(jsonObject.membership_id);
            $("#previous").val(jsonObject.balance);
            $("#available_points").val(jsonObject.available_points);
            $("#max_available_points").val(jsonObject.available_points);
        },
...
function onselectimage(id) {
    ...
    if (parseInt(stock) > 0) {
        ...
        var membership_id = $("#membership_id").val() ? $("#membership_id").val() : 0;
        if (product_id == exist) {
