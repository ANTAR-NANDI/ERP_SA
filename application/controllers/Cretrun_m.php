...
class Cretrun_m extends CI_Controller
{
    ...
    public function invoice_return_form_c($invoice_id)
    {
        ...
        // For new POS Sale Return
        $content = $CI->lreturn->invoice_return_data_new($invid->invoice_id);
        $this->template->full_admin_html_view($content);
    }

    // supplier return form
    public function supplier_return_form($purchase_id)
    {
        $purchase_id = $this->input->post('purchase_id', TRUE) ? $this->input->post('purchase_id', TRUE) : $purchase_id;
    ...
    public function return_invoice()
    {
        ...
        // exit();
        $invoice_id = $CI->Returnse->return_invoice_entry_new_2();
