...
class Lreport extends CI_Model
{
    ...
    public function daywise_sales_report()
    {
        $CI = &get_instance();
        $CI->load->model('Reports');
        $sales_data = $CI->Reports->daywise_sales_report();
        $thirtyDaysAgo = date('Y-m-d', strtotime('-30 days'));

        // Select the product-wise date-wise sum of sold quantities for the last 30 days
        $this->db->select('product_information.product_name, DATE(invoice.date) as sale_date, SUM(invoice_details.quantity) as total_sold_quantity')
                 ->from('invoice_details')
                 ->join('product_information', 'product_information.product_id = invoice_details.product_id', 'left')
                 ->join('invoice', 'invoice.invoice_id = invoice_details.invoice_id', 'left')
                 ->where('invoice.date >=', $thirtyDaysAgo)
                 ->group_by('product_information.product_id, DATE(invoice.date)');
        ...
        foreach ($sales_data as $item) {
            ...
            $totalSalesQnty = $item["totalSalesQnty"];

            if (!isset($outputArray[$saleDate])) {
                $outputArray[$saleDate] = array();
            }

            $outputArray[$saleDate][] = array(
                "product_name" => $productName,
                "totalSalesQnty" => $totalSalesQnty
            );
        }
        $data['sales_data'] = $outputArray;
