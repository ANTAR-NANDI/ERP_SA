...
class Lreturn
{
    ...
    // For new pos sale
    public function invoice_return_data_new($invoice_id)
    {
        $CI = &get_instance();
        $CI->load->model('Invoices');
        $CI->load->model('Courier');
        $CI->load->model('Web_settings');
        $CI->load->model('Settings');
        $invoice_detail = $CI->Invoices->retrieve_invoice_editdata_new($invoice_id);
        ...
        if (!empty($invoice_detail)) {
            foreach ($invoice_detail as $k => $v) {
                $i++;
                $invoice_detail[$k]['sl'] = $i;
                $invoice_detail[$k]['rate'] =  number_format($invoice_detail[$k]['rate'], 2, '.', '');
            }
        }
        $agg_id = $invoice_detail[0]['agg_id'];
        $outlet_id = $invoice_detail[0]['outlet_id'];
        ...
        $data = array(
            ...
            'receiver_name'         => $invoice_detail[0]['receiver_name'],
            'total_amount'  => $invoice_detail[0]['total_amount'],
            'net_total'  => $invoice_detail[0]['net_total'],
            'paid_amount'   => $invoice_detail[0]['p_amnt'],
            'due_amount'    => $invoice_detail[0]['due_amount'],
            'total_discount' => $invoice_detail[0]['total_discount'],
            'invoice_discount' => $invoice_detail[0]['invoice_discount'],
        ...
        // print_r($data);
        // exit();
        $chapterList = $CI->parser->parse('return/return_data_form_new', $data, true);
        return $chapterList;
    }
    ...
    public function supplier_return_data($purchase_id)
    {
        ...
        if (!empty($purchase_detail)) {
            foreach ($purchase_detail as $k => $v) {
                ...
                $purchase_detail[$k]['discount'] = number_format($purchase_detail[$k]['discount'], 2, '.', ',');
            }
        }
    ...
    public function supplier_html_data($purchase_id)
    {
        ...
        if (!empty($return_detail)) {
            foreach ($return_detail as $k => $v) {
                ...
                $return_detail[$k]['discount'] = ($return_detail[$k]['ret_qty'] * $return_detail[$k]['price'] )- $return_detail[$k]['total_ret_amount'];
                ...
                $subTotal_discount = $subTotal_discount + $return_detail[$k]['discount'];
                $subTotal = $subTotal + ($return_detail[$k]['price'] * $return_detail[$k]['ret_qty']);
        ...
        $data = array(
            ...
            'subTotal_quantity' => $subTotal_quantity,
            'subTotal_discount' => number_format($subTotal_discount, 2, '.', ','),
